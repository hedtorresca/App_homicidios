...
tabla_n4 <- function(datos, categorias, variable,  mensaje, titulo){
  sketch = htmltools::withTags(table(
    class = 'display',
    thead(
      tr(
  ...
  tba1 <- datos
  tba2 <- data.frame(apply(tba1[,-4],2,as.factor))
  tba <- datatable(
    tba2, 
    fillContainer = paste0("Tabla: ",mensaje),
    filter = 'top',
...
tabla_n2 <- function(datos, categorias, variable,  mensaje, titulo){
  ...
  tba1 <- datos
  tba2 <- data.frame(apply(tba1[,-2],2,as.factor))
  tba <- datatable(
    tba2, 
    fillContainer = paste0("Tabla: ",mensaje),
    filter = 'top',
...
barra_vertical <- function(datos,categoria,colores,ano,periodo,periodo_titulo,titulo,eje){
  ...
  gfa <- highchart()  %>%
    ...
    hc_title(style = list( fontWeight = "bold",fontSize = "25px" ), text = paste0(titulo)) %>%
